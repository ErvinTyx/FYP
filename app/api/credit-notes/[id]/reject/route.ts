/**
 * PUT /api/credit-notes/[id]/reject
 * Body: { reason: string }. Reject a credit note (status must be Pending Approval).
 */
  ...
  try {
    ...
    if (!canReject) {
      return NextResponse.json(
        { success: false, message: 'Forbidden: You do not have permission to reject credit notes' },
        { status: 403 }
      );
    }
    ...
    if (cn.status !== 'Pending Approval') {
      return NextResponse.json(
        { success: false, message: 'Only credit notes with status Pending Approval can be rejected' },
        { status: 400 }
      );
    }
    ...
    console.error('[Credit notes] reject error:', error);
    return NextResponse.json(
      { success: false, message: 'Failed to reject credit note' },
      { status: 500 }
    );
  }
}
