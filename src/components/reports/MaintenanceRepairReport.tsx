...
import { Download, Wrench, FileSpreadsheet, Loader2, Play } from 'lucide-react';
...
export function MaintenanceRepairReport({ filters }: { filters: ReportFilter }) {
  ...
  const generateReport = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await fetch('/api/reports/maintenance-repair');
